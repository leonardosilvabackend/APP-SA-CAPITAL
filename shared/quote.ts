...
export function commercialQuoteText(quotas: CalculationQuota[], commissionRate: number) {
  ...
  const categories = Array.from(new Set(quotas.map(quota => quota.category.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase()))).join(" / ");
  const administrators = Array.from(new Set(quotas.map(quota => quota.administrator))).join(" / ");
  return `ADMINISTRADORA: ${administrators} - ${categories}\n\nCREDITO: ${money(summary.creditTotal)}\nENTRADA: ${money(summary.finalEntryTotal)} — ${summary.entryPercentage.toLocaleString("pt-BR", {minimumFractionDigits:2, maximumFractionDigits:2})}%\n\nPRAZO/PARCELA:\n\n${summary.installmentCascade.map(item => `${item.from}º a ${item.to}: ${money(item.amount)}`).join("\n")}\n\nSALDO DEVEDOR: ${money(summary.outstandingBalanceTotal)}\nTAXA DE TRANSFERÊNCIA: ${money(summary.transferFeeTotal)}\nSEGURO DE VIDA: ${money(summary.insuranceTotal)}\n\nCartas selecionadas:\n\n${quotas.map(quota => `${quota.code}: ${money(number(quota.creditAmount))}`).join("\n")}`;
}
