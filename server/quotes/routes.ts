...
export async function cleanupExpiredQuotes() { const db = getDatabase(); if (db) await db.delete(savedQuotes).where(and(lt(savedQuotes.expiresAt, new Date()), sql`not exists (select 1 from jsonb_array_elements(${savedQuotes.selectedQuotas}) saved_quota where not exists (select 1 from ${quotas} current_quota where current_quota.id::text = saved_quota->>'id' and current_quota.status = 'available'))`)); }
