...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronLeft, ChevronRight, Download, Pencil, Plus, Search, Star, Upload } from "lucide-react";
...
import { downloadSaStock, stockApi, stockCurrency as currency, type Opportunity, type QuotaRecord, type StockFilters, type StockResponse } from "./stock/contracts";
...
export default function StockPage({ user }: { user: AuthenticatedUser }) {
  ...
  const canImportStock = ["admin", "advisor"].includes(user.role);
  ...
  return <section className="stock-page"><div className="stock-advisor-row"><AdvisorContact /></div><div className="page-heading-row"><div><span className="eyebrow">OPERAÇÃO COMERCIAL</span><h1>Estoque de cotas</h1><p>{stock.data?.total ?? 0} condições encontradas.</p></div><div className="heading-actions">{!!opportunities.data?.items.length && <button className="opportunity-button" onClick={() => setShowOpportunities(true)}><Star size={18} fill="currentColor" /> Oportunidades do dia!</button>}<button className="primary-button button-reset smart-order-button" onClick={() => setSmartSearching(true)}><Sparkles size={19} /> Pedido Inteligente</button>{canImportStock && <><button className="secondary-button" disabled={exportStock.isPending} onClick={() => exportStock.mutate()}><Download size={17} /> {exportStock.isPending ? "Gerando…" : "Baixar planilha"}</button><button className="secondary-button" onClick={() => setImporting(true)}><Upload size={17} /> Importar planilha</button></>}{isAdmin && <button className="primary-button button-reset" onClick={() => setCreating(true)}><Plus size={18} /> Nova cota</button>}</div></div>
